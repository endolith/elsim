...
def test_basic(tiebreaker):
    ...
    assert approval(election, tiebreaker) == 1  # Nashville
    ...
    assert approval(election, tiebreaker) == 1  # McCain
    ...
    assert approval(election, tiebreaker) == 0  # A
...
def test_ties():
    ...
    # No tiebreaker:
    assert approval(election, tiebreaker=None) is None

    # Mode 'order' should always prefer lowest candidate ID
    assert approval(election, tiebreaker='order') == 1

    # Mode 'random' should choose all tied candidates at random
    assert collect_random_results(approval, election) == {1, 2}
    ...
    # Mode 'order' should always prefer lowest candidate ID
    assert approval(election, tiebreaker='order') == 0
...
def test_legit_winner(election, tiebreaker):
    ...
    winner = approval(election, tiebreaker)
    assert isinstance(winner, int)
