...
def test_irv_rounds_returns_none_for_an_unbroken_elimination_tie():
    """A trace must not imply a transfer without a tie-breaking rule."""
    election = np.array([
        [0, 1, 2],
        [1, 2, 0],
        [2, 0, 1],
...
def test_irv_rounds_rejects_out_of_range_stop_counts(stop_at):
    """Invalid stopping counts would create incomplete trace states."""
    election = np.array([
        [0, 1, 2],
        [1, 2, 0],
    ])
...
def test_irv_rounds_rejects_non_integer_stop_counts():
    """A fractional stopping count has no meaningful interpretation."""
    election = np.array([
        [0, 1],
        [1, 0],
    ])
