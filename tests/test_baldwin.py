...
def test_baldwin_and_tvr_both_stop_at_a_new_majority():
    """Both methods stop once a transfer creates a first-choice majority."""
    election = _example_election()

    baldwin_result = baldwin_rounds(election)
    tvr_result = total_vote_runoff_rounds(election)

    assert len(baldwin_result.rounds) == 1
