...
def vote_for_or_against_k(utilities, k, rng=None):
    ...
    ballots = _np.zeros((n_voters, n_cands), dtype=_np.int8)
    rows = _np.arange(n_voters)[:, _np.newaxis]

    u = utilities.astype(_np.float64, copy=False)
    u_j = u + rng.random(u.shape) * (_np.finfo(_np.float64).eps * 64)
    top_k = _np.argpartition(u_j, -k, axis=1)[:, -k:]
    bot_k = _np.argpartition(u_j, k - 1, axis=1)[:, :k]
    choice = rng.integers(2, size=n_voters, dtype=_np.int8)
    vote_for = (choice == 0)[:, _np.newaxis]
    target = _np.where(vote_for, top_k, bot_k)
    vals = _np.where(vote_for, _np.int8(1), _np.int8(-1))
