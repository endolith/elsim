...
def validate_center_outward(result, candidates, election):
    ...
    if condorcet_winner is None or not result.rounds:
        return None

    candidates = np.asarray(candidates)
    distances = np.linalg.norm(candidates, axis=1)
    active = set(range(len(candidates)))
    if result.rounds[0].eliminated != condorcet_winner:
        return None
...
def simulate_irv_rounds(election, candidates):
    """Run IRV and verify it eliminates the Condorcet winner first."""
...
def find_center_outward_election(n_voters, n_cands, max_trials, disp=1.0):
    """Sample elections where IRV squeezes out the Condorcet winner."""
