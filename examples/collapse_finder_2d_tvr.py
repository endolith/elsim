"""Find and animate a 2D Baldwin center-convergent election."""
...
def validate_center_convergence(result, candidates):
    """Return ``result`` when Baldwin elects the nearest-to-origin candidate."""
    if result is None:
        return None
    candidates = np.asarray(candidates)
    center_candidate = int(np.argmin(np.linalg.norm(candidates, axis=1)))
    if result.winner != center_candidate:
        return None
    return result


def simulate_tvr_rounds(election, candidates):
    """Run the full Baldwin trace and keep geometric-center winners."""
    return validate_center_convergence(
...
def find_center_convergent_election(n_voters, n_cands, max_trials, disp=1.0):
    """Sample elections until Baldwin elects the nearest-to-origin candidate."""
...
if __name__ == '__main__':
    ...
    if result is None:
        raise RuntimeError(
            'No center-convergent Baldwin election found. '
            'Increase max_trials or reduce n_cands.'
        )

    trial, voters, candidates, rankings, trace = result
    print(f'Found center-convergent Baldwin election on trial {trial}.')
    print(
        'Elimination order:',
