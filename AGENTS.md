...
- **Don't expand a PR's scope because a reviewer pointed something out.** Reviewer comments may flag pre-existing or out-of-scope issues; if a comment isn't about code this PR changed, move it to its own PR (or reply noting it's out of scope) rather than folding it into the PR under review. Only fix comments that are about the PR's own changes.
